...
class ApiService {
    ...
    this.client.interceptors.response.use(
      ...
      (error) => {
        if (error.response?.status === 401) {
          this.clearToken();
          // Only redirect to login if we're not on the home page and it's not a silent auth check
          const isHomePage = typeof window !== 'undefined' &&
            (window.location.pathname === '/' || window.location.pathname === '');
          const isSilentCheck = error.config?.headers?.['X-Silent-Auth'] === 'true';

          if (!isHomePage && !isSilentCheck) {
  ...
  async login(data: LoginRequest): Promise<AuthResponse> {
    ...
    // Backend wraps response in {data: {...}, meta: {}} format
  ...
  async getCurrentUserSilent(): Promise<User> {
    const response = await this.client.get('/auth/me', {
      headers: {
        'X-Silent-Auth': 'true'
      }
