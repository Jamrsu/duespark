...
from app.models import User, Client, Invoice, InvoiceStatus, Subscription
...
class ClientPortalService:
    ...
    def generate_client_access_token(self, client: Client, invoice_id: Optional[int] = None) -> str:
        """Generate secure access token for client portal"""
        # Create unique token data
        token_data = f"{client.id}:{client.email}:{datetime.now(timezone.utc).isoformat()}"
        if invoice_id:
    ...
    def verify_client_access(self, token: str, client_email: str) -> Tuple[bool, Optional[Client]]:
    ...
    def get_client_portal_data(self, client: Client, access_token: str) -> Dict:
            ...
            and_(
                ...
                or_(
                    ...
                    and_(
                        ...
                        Invoice.updated_at >= datetime.now(timezone.utc) - timedelta(days=30)
                    )
                )
            )
        ...
        return {
            ...
            "summary": {
                ...
                "invoice_count": len([inv for inv in invoices if inv.status in [InvoiceStatus.sent, InvoiceStatus.overdue]]),
    ...
    def get_invoice_payment_page(self, invoice_id: int, access_token: str) -> Dict:
        ...
        # Verify access
        is_valid, verified_client = self.verify_client_access(access_token, client.email)
        ...
        return {
            ...
            "payment": {
                ...
                "processing_fee": self._calculate_processing_fee(invoice.amount, subscription.tier),
                "total_amount": self._calculate_total_with_fees(invoice.amount, subscription.tier),
    ...
    def process_payment(self, invoice_id: int, payment_data: Dict, access_token: str) -> Dict:
    ...
    def send_client_message(self, client: Client, vendor: User, message: str, invoice_id: Optional[int] = None) -> Dict:
    ...
    def get_client_communication_history(self, client: Client, access_token: str) -> List[Dict]:
    ...
    def _format_invoice_for_client(self, invoice: Invoice) -> Dict:
        ...
        return {
            ...
            "days_until_due": (invoice.due_date - datetime.now(timezone.utc).date()).days,
    ...
    def _get_last_payment_date(self, client: Client) -> Optional[str]:
        ...
        last_paid = self.db.query(Invoice).filter(
            and_(
                Invoice.client_id == client.id,
                Invoice.status == InvoiceStatus.paid,
        ...
        return last_paid.paid_at.isoformat() if last_paid and last_paid.paid_at else None
