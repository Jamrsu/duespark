...
from .models import BillingEvent, Subscription, SubscriptionStatus, SubscriptionTier
...
def handle_subscription_deleted(event_data: dict, db: Session):
    ...
    if subscription:
        ...
        subscription.ended_at = datetime.fromtimestamp(
            subscription_data.get("ended_at", int(datetime.utcnow().timestamp()))
        )
        db.commit()
        ...
        if subscription.first_payment_at:
            ...
            if days_since_first_payment < 30:
                referral_service.revoke_referral_reward(
                    subscription.user_id,
                    "Subscription canceled within 30 days of first payment",
                    db,
                )
...
def handle_invoice_payment_succeeded(event_data: dict, db: Session):
    ...
    if subscription_id:
        ...
        if subscription:
            ...
            is_first_payment = (
                subscription.first_payment_at is None
                and subscription.tier != SubscriptionTier.freemium
                and invoice_data.get("amount_paid", 0) > 0
            )
            ...
            if is_first_payment:
                ...
                referral_service.check_and_grant_referral_reward(
...
def handle_checkout_session_completed(event_data: dict, db: Session):
    ...
    if customer_id and user_id:
        subscription = (
            db.query(Subscription).filter(Subscription.user_id == int(user_id)).first()
        )
...
async def stripe_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    if event_id:
        ...
        if billing_event and getattr(billing_event, "processed", False):
