...
from app.models import User, Client, Invoice, InvoiceStatus, SubscriptionTier
...
class AnalyticsService:
    ...
    def get_user_analytics(self, user: User) -> Dict:
        ...
        try:
            # Basic usage stats
            total_invoices = self.db.query(Invoice).filter(Invoice.user_id == user.id).count()
            total_clients = self.db.query(Client).filter(Client.user_id == user.id).count()
            ...
            total_revenue = sum(float(inv.amount) for inv in paid_invoices) if paid_invoices else 0.0
            avg_invoice_value = total_revenue / len(paid_invoices) if paid_invoices else 0.0
    ...
    def get_client_insights(self, user: User, limit: int = 10) -> List[ClientInsight]:
        """Generate insights for user's clients"""
        try:
            clients = self.db.query(Client).filter(Client.user_id == user.id).limit(limit).all()
            insights = []

            for client in clients:
                insight = self._analyze_client_behavior(client)
                insights.append(insight)

            # Sort by risk level and payment behavior
            insights.sort(key=lambda x: (x.risk_level.value, x.avg_payment_delay), reverse=True)
            return insights
    ...
    def predict_cash_flow(self, user: User, days_ahead: int = 30) -> CashFlowPrediction:
        ...
        try:
            ...
            for week in range(0, days_ahead, 7):
                ...
                week_invoices = [inv for inv in pending_invoices
                ...
                adjusted_amount = week_amount * self._calculate_payment_probability(user)
    ...
    def track_feature_usage(self, user: User, feature: str, metadata: Optional[Dict] = None):
        """Track feature usage for analytics (would store in database in production)"""
        try:
            # In production, store this in a feature_usage table
            logger.info(f"Feature usage: {user.id} used {feature} with metadata {metadata}")
    ...
    def _analyze_client_behavior(self, client: Client) -> ClientInsight:
        ...
        try:
            ...
            invoices = self.db.query(Invoice).filter(Invoice.client_id == client.id).all()
            ...
            last_payment = max((inv.paid_at for inv in paid_invoices if inv.paid_at), default=None)
            ...
            return ClientInsight(
                ...
                risk_level=risk,
                predicted_payment_date=self._predict_next_payment_date(client, avg_delay),
    ...
    def _analyze_tier_usage(self, user: User) -> Dict:
        ...
        current_clients = self.db.query(Client).filter(Client.user_id == user.id).count()
        ...
        return {
            ...
            "clients": {
                ...
                "utilization": min(current_clients / limits.get("max_clients", 999999), 1.0) if limits.get("max_clients") != 999999 else 0.0
            },
            "monthly_invoices": {
                "used": monthly_invoices,
                "limit": limits.get("max_invoices_per_month", "unlimited"),
                "utilization": min(monthly_invoices / limits.get("max_invoices_per_month", 999999), 1.0) if limits.get("max_invoices_per_month") != 999999 else 0.0
            },
            "ai_features_enabled": limits.get("ai_features", False),
            "upgrade_recommended": self._should_recommend_upgrade(user, current_clients, monthly_invoices)
        }
    ...
    def _calculate_performance_metrics(self, user: User) -> Dict:
        ...
        if not invoices:
            return {"payment_rate": 0.0, "avg_collection_time": 0.0, "efficiency_score": 0.0}
        ...
        avg_collection_time = sum(collection_times) / len(collection_times) if collection_times else 0.0

        # Calculate efficiency score (0.0 to 1.0)
        efficiency_score = min(payment_rate * (1.0 - min(avg_collection_time / 30.0, 1.0)), 1.0)
    ...
    def _calculate_growth_trends(self, user: User) -> Dict:
        ...
        recent_avg = sum(d["invoices"] for d in monthly_data[:3]) / 3 if len(monthly_data) >= 3 else 0
        older_avg = sum(d["invoices"] for d in monthly_data[3:]) / 3 if len(monthly_data) >= 6 else 0

        growth_rate = ((recent_avg - older_avg) / older_avg * 100) if older_avg > 0 else 0.0
    ...
    def _calculate_prediction_confidence(self, user: User, pending_invoices: List[Invoice]) -> float:
    ...
    def _identify_cash_flow_risks(self, user: User, pending_invoices: List[Invoice]) -> List[str]:
        """Identify potential cash flow risks"""
        risks = []

        overdue_count = len([inv for inv in pending_invoices if inv.status == InvoiceStatus.overdue])
        if overdue_count > 0:
        ...
        for invoice in pending_invoices:
            ...
            client_concentrations[client_id] = client_concentrations.get(client_id, 0.0) + amount
            total_amount += amount

        if total_amount > 0:
            max_client_percent = max(client_concentrations.values()) / total_amount
            if max_client_percent > 0.5:
                risks.append("High concentration risk - significant revenue from single client")
    ...
    def _identify_cash_flow_opportunities(self, user: User) -> List[str]:
        ...
        tier = self.subscription_gate.get_user_tier(user)
        if tier == SubscriptionTier.freemium:
            opportunities.append("Upgrade to Professional for advanced collection tools")

        return opportunities

    def _calculate_client_risk(self, client: Client, overdue_count: int, avg_delay: float) -> RiskLevel:
    ...
    def _predict_next_payment_date(self, client: Client, avg_delay: float) -> Optional[datetime]:
        """Predict when client will make their next payment"""
        # Find the most recent unpaid invoice
        recent_invoice = self.db.query(Invoice).filter(
            and_(
    ...
    def _generate_client_specific_recommendations(self, client: Client, behavior: PaymentBehavior,
                                                 risk: RiskLevel, avg_delay: float) -> List[str]:
    ...
    def _generate_client_recommendations(self, user: User) -> List[Dict]:
        ...
        high_risk_clients = self.db.query(Client).filter(Client.user_id == user.id).limit(5).all()
    ...
    def _should_recommend_upgrade(self, user: User, current_clients: int, monthly_invoices: int) -> bool:
    ...
    def _get_default_cash_flow_prediction(self, user: User, days_ahead: int) -> CashFlowPrediction:
