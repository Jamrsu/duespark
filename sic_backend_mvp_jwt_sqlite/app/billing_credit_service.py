...
from .models import User, Subscription, SubscriptionCredit, BillingEvent, SubscriptionTier
...
class BillingCreditService:
    ...
    def calculate_credit_application(self, user_id: int, invoice_amount_cents: int, db: Session) -> Dict:
        """
        ...
        available_credits = db.query(SubscriptionCredit).filter(
        ...
        total_available_cents = sum(credit.remaining_amount_cents for credit in available_credits)
        ...
        for credit in available_credits:
            ...
            if credit.remaining_amount_cents >= remaining_to_apply:
                ...
                credits_used.append({
                    'credit': credit,
                    'amount_to_consume': remaining_to_apply
                })
    ...
    def apply_credits_to_invoice(self, user_id: int, invoice_amount_cents: int, db: Session, description: str = None) -> Dict:
        """
        ...
        """
        calculation = self.calculate_credit_application(user_id, invoice_amount_cents, db)
        ...
        success = referral_service.consume_credits_cents(user_id, calculation['credits_to_apply'], db)
    ...
    def create_checkout_session_with_credits(self, user_id: int, tier: SubscriptionTier, db: Session) -> Dict:
        """
    ...
    def get_billing_preview(self, user_id: int, tier: SubscriptionTier, db: Session) -> Dict:
        """
        ...
        return {
            ...
            'original_price_display': pricing_service.format_price_display(tier_price_cents),
            'available_credits_cents': calculation['total_available_credits'],
            'available_credits_display': pricing_service.format_price_display(calculation['total_available_credits']),
            'credits_to_apply_cents': calculation['credits_to_apply'],
            'credits_to_apply_display': pricing_service.format_price_display(calculation['credits_to_apply']),
            'final_amount_cents': calculation['remaining_invoice_amount'],
            'final_amount_display': pricing_service.format_price_display(calculation['remaining_invoice_amount']),
            'fully_covered_by_credits': calculation['remaining_invoice_amount'] == 0
        }

    def handle_successful_payment_with_credits(self, user_id: int, invoice_amount_cents: int, db: Session):
        """
