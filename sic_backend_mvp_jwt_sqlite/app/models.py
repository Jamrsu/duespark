...
from sqlalchemy import Column, Integer, String, DateTime, ForeignKey, Enum, Text, Numeric, Date, func, JSON, Boolean
...
class User(Base):
    ...
    subscription_credits = relationship("SubscriptionCredit", back_populates="user", cascade="all, delete-orphan")
...
class Payment(Base):
    ...
    payment_method: Mapped[str] = mapped_column(String(50), nullable=True)  # 'stripe', 'manual', etc.
...
class Subscription(Base):
    ...
    tier: Mapped[SubscriptionTier] = mapped_column(Enum(SubscriptionTier, name="subscriptiontier"), default=SubscriptionTier.freemium, nullable=False)
    status: Mapped[SubscriptionStatus] = mapped_column(Enum(SubscriptionStatus, name="subscriptionstatus"), default=SubscriptionStatus.active, nullable=False)
    ...
    # Pause functionality
    is_paused: Mapped[bool] = mapped_column("paused", Integer, default=0, nullable=False)
    ...
    reminders_sent_count: Mapped[int] = mapped_column("reminders_sent_this_period", Integer, default=0, nullable=False)
...
class UsageLimits(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    tier: Mapped[SubscriptionTier] = mapped_column(Enum(SubscriptionTier, name="subscriptiontier"), unique=True, nullable=False)

    # Plan limits
    reminders_per_month: Mapped[int] = mapped_column(Integer, nullable=False)
    max_clients: Mapped[int | None] = mapped_column("clients_limit", Integer, nullable=True)  # None = unlimited
    max_invoices_per_month: Mapped[int | None] = mapped_column("invoices_limit", Integer, nullable=True)  # None = unlimited
    ...
    api_requests_per_hour: Mapped[int] = mapped_column(Integer, nullable=False, default=100)
...
class BillingEvent(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), index=True, nullable=False)
    subscription_id: Mapped[int | None] = mapped_column(ForeignKey("subscriptions.id", ondelete="SET NULL"), index=True, nullable=True)

    # Event details
    event_type: Mapped[str] = mapped_column(String(64), nullable=False)  # e.g., subscription.created, invoice.paid, etc.
    stripe_event_id: Mapped[str | None] = mapped_column(String(255), unique=True, index=True, nullable=True)
...
class Referral(Base):
    ...
    referral_code_used: Mapped[str] = mapped_column(String(16), index=True, nullable=False)
...
class SubscriptionCredit(Base):
    ...
    remaining_amount_cents: Mapped[int] = mapped_column(Integer, nullable=False, default=0)

    source: Mapped[str] = mapped_column(String(32), index=True, nullable=False)  # 'referral', 'admin_grant', 'promo'
    source_id: Mapped[str | None] = mapped_column(String(64), nullable=True)  # referral_id, admin_user_id, promo_code
