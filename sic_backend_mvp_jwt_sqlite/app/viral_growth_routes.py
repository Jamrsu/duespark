...
from app.viral_growth_service import ViralGrowthService, get_viral_growth_service
...
async def generate_share_content(
    ...
    try:
        ...
        share_content = growth_service.generate_share_content(
            current_user, request.platform, request.template_type, **template_vars
        )

        return {"success": True, "data": share_content}
...
async def create_team_invitation(
    ...
    try:
        ...
        invitation = growth_service.create_team_invitation(
            current_user, request.email, request.role
        )
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "current_level": growth_service._calculate_user_level(
                    stats["referrals"]["successful"]
                ),
...
async def track_share_action(
    request: ShareTrackingRequest, current_user: User = Depends(get_current_user)
):
