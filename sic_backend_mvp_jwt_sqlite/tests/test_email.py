...
def test_preview_and_send_now(monkeypatch):
    ...
    rc = client.post(
        "/clients",
        headers=headers,
        json={"name": "Alpha Co", "email": f"{uuid.uuid4().hex[:6]}@ex.com"},
    )
    ...
    rp = client.post(
        "/reminders/preview",
        headers=headers,
        json={"invoice_id": inv_id, "template_id": tid, "tone": "neutral"},
    )
    ...
    sn3 = client.post(
        "/reminders/send-now", headers=headers, json={"reminder_id": rid, "force": True}
    )
