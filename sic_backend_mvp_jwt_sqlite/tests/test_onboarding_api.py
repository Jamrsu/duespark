...
def create_test_user(db: Session, email: str = "test@example.com", onboarding_status: str = "not_started"):
...
def test_send_verification_email(test_db):
    ...
    response = client.post("/auth/send-verification", json=request_data, headers=headers)
...
def test_unauthorized_access_to_onboarding_endpoints(test_db):
    ...
    response = client.post("/auth/send-verification", json={"email": "test@example.com"})
    assert response.status_code == 401
...
def test_events_isolation_between_users(test_db):
    ...
    event1 = models.Event(user_id=user1.id, entity_type="user", entity_id=user1.id, event_type="user1_event")
    event2 = models.Event(user_id=user2.id, entity_type="user", entity_id=user2.id, event_type="user2_event")
...
def test_sample_data_isolation_between_users(test_db):
    ...
    user1_clients = db.query(models.Client).filter(models.Client.user_id == user1.id).all()
    assert len(user1_clients) == 3

    # Verify user2 has no data
    user2_clients = db.query(models.Client).filter(models.Client.user_id == user2.id).all()
...
def test_onboarding_status_enum_validation(test_db):
    ...
    # Valid status
    valid_statuses = ["not_started", "account_created", "email_verified", "payment_configured", "completed"]
