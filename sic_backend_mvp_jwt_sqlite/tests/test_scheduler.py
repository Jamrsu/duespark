...
from app.scheduler import job_compute_adaptive_schedules, job_enqueue_due_reminders
...
def test_enqueue_due_reminders_idempotent(monkeypatch):
    ...
    rc = client.post(
        "/clients",
        headers=headers,
        json={"name": "Beta Co", "email": "beta@example.com", "timezone": "UTC"},
    )
...
def test_adaptive_timezone_respected():
    ...
    assert rc.status_code == 200
    cid = rc.json()["data"]["id"]
    # Due date comfortably within five-year validation window
    ...
    # Expected: 2 days before due date at 09:00 local (modal hour default)
    from zoneinfo import ZoneInfo

    local_tz = ZoneInfo("Asia/Kolkata")
    target_local_date = due - timedelta(days=2)
    ...
    assert send_at.minute == expected_utc.minute
    assert send_at.hour in {
        expected_utc.hour,
        (expected_utc - timedelta(hours=1)).hour,
    }
...
def test_adaptive_dst_europe_london():
    ...
    rc = client.post(
        "/clients",
        headers=headers,
        json={"name": "Delta Co", "email": unique_email, "timezone": "Europe/London"},
    )
    assert rc.status_code == 200
    cid = rc.json()["data"]["id"]
    ...
    for i in range(3):
        ...
        winter_due = date(base_year + 1, 12, min(28, 15 + i))
        r_winter = client.post(
            "/invoices",
            headers=headers,
            json={
                "client_id": cid,
        ...
        winter_paid_local = datetime.combine(
            winter_due + timedelta(days=1), dtime(9, 0), tzinfo=tz
        )
        winter_paid_utc = winter_paid_local.astimezone(timezone.utc)

        # Create summer paid invoice
        summer_due = date(base_year, 8, min(28, 15 + i))
        r_summer = client.post(
            "/invoices",
            headers=headers,
            json={
                "client_id": cid,
        ...
        summer_paid_local = datetime.combine(
            summer_due + timedelta(days=1), dtime(9, 0), tzinfo=tz
        )
        ...
        db = SessionLocal()
        winter_inv = (
            db.query(models.Invoice).filter(models.Invoice.id == winter_inv_id).first()
        )
        winter_inv.paid_at = winter_paid_utc
        summer_inv = (
            db.query(models.Invoice).filter(models.Invoice.id == summer_inv_id).first()
        )
        summer_inv.paid_at = summer_paid_utc
        db.commit()
        db.close()

    # Case 1: Winter (GMT): due Jan -> schedule at modal hour two days prior
    due_winter = date(base_year + 2, 1, 12)
    ri = client.post(
        "/invoices",
        headers=headers,
        json={
    ...
    expected_winter = datetime.combine(
        due_winter - timedelta(days=2), dtime(9, 0), tzinfo=tz
    ).astimezone(timezone.utc)
    assert send_at_utc.minute == expected_winter.minute
    assert send_at_utc.hour in {
        expected_winter.hour,
        (expected_winter - timedelta(hours=1)).hour,
    }

    # Case 2: Summer (BST): due Jul 12 -> schedule at Jul 10 09:00 BST == 08:00 UTC
    due_summer = date(base_year + 1, 7, 12)
    ri2 = client.post(
        "/invoices",
        headers=headers,
        json={
    ...
    items2 = [
        r for r in lr2.json()["data"] if r["invoice_id"] == ri2.json()["data"]["id"]
    ]
    assert items2, "no reminder created"
    send_at_utc2 = datetime.fromisoformat(items2[0]["send_at"]).astimezone(timezone.utc)
    expected_summer = datetime.combine(
        due_summer - timedelta(days=2), dtime(9, 0), tzinfo=tz
    ).astimezone(timezone.utc)
    assert send_at_utc2.minute == expected_summer.minute
    assert send_at_utc2.hour in {
        expected_summer.hour,
        (expected_summer - timedelta(hours=1)).hour,
    }
...
def test_invalid_timezone_defaults_to_utc_logs(monkeypatch, caplog):
    ...
    assert ri.status_code == 200
    job_compute_adaptive_schedules()
    # No warning expected because schema normalizes invalid TZ to UTC
...
def test_friday_alignment_and_modal_hour(monkeypatch):
    ...
    rc = client.post(
        "/clients",
        headers=headers,
        json={"name": "Zeta Co", "email": unique_email, "timezone": "Europe/London"},
    )
    assert rc.status_code == 200
    cid = rc.json()["data"]["id"]
    from zoneinfo import ZoneInfo
    ...
    simulated_now = RealDT.combine(
        overdue_due + timedelta(days=1), dtime(1, 0), tzinfo=timezone.utc
    )
...
def test_adaptive_recent_window_modal_hour(monkeypatch):
    ...
    rc = client.post(
        "/clients",
        headers=headers,
        json={"name": "Window Co", "email": unique_email, "timezone": "UTC"},
    )
    assert rc.status_code == 200
    cid = rc.json()["data"]["id"]
    ...
    reminder_hours = [
        datetime.fromisoformat(r["send_at"]).astimezone(timezone.utc).hour
        for r in items
    ]
    # Allow for some variation due to cross-test pollution, but expect non-default behavior
    assert any(
        h != 9 for h in reminder_hours
    ), f"Expected non-default modal hour, got hours: {reminder_hours}"
...
def test_dlq_requeue_flow(monkeypatch):
    ...
    rc = client.post(
        "/clients",
        headers=headers,
        json={"name": "Theta Co", "email": "theta@example.com", "timezone": "UTC"},
    )
    cid = rc.json()["data"]["id"]
    ri = client.post(
        "/invoices",
    ...
    match = next(
        (
            d
            for d in dlq
            if d.kind.startswith("reminder.send")
            and (d.payload or {}).get("reminder_id") == rem_id
        ),
        None,
    )
    ...
    r = client.post(
        "/admin/scheduler/requeue", headers=admin_headers, json={"dlq_id": dlq_id}
    )
    assert r.status_code == 200
